...
router.post('/', authenticate, async (req, res, next) => {
  try {
    ...
    // 将创建者加入家庭并设为管理员
    await execute(
      'UPDATE users SET family_id = $1::text, role = $1 WHERE id = $1::text',
...
router.get('/current', authenticate, async (req, res, next) => {
  try {
    ...
    const families = await query(
      'SELECT * FROM families WHERE id = $1::text',
      [req.user.familyId]
    );
    ...
    const members = await query(
      'SELECT COUNT(*) as count FROM users WHERE family_id = $1::text',
      [req.user.familyId]
    );

    // 获取家庭成员列表
    const memberList = await query(
      'SELECT id, username, nickname, avatar, role, created_at FROM users WHERE family_id = $1::text',
      [req.user.familyId]
    );
...
router.put('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    params.push(id);
    await execute(
      `UPDATE families SET ${updates.join(', ')} WHERE id = $1::text`,
      params
    );
...
router.post('/invite', authenticate, async (req, res, next) => {
  try {
    ...
    // 检查是否是管理员
    const families = await query(
      'SELECT admin_id, invite_code FROM families WHERE id = $1::text',
      [req.user.familyId]
    );
    ...
    await execute(
      'UPDATE families SET invite_code = $1 WHERE id = $1::text',
...
router.post('/join', authenticate, async (req, res, next) => {
  try {
    ...
    // 将用户加入家庭
    await execute(
      'UPDATE users SET family_id = $1::text WHERE id = $1::text',
...
router.post('/leave', authenticate, async (req, res, next) => {
  try {
    ...
    // 退出家庭
    await execute(
      'UPDATE users SET family_id = NULL WHERE id = $1::text',
      [req.user.id]
    );
...
router.delete('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    await execute('DELETE FROM families WHERE id = $1::text', [id]);
...
router.delete('/:id/members/:memberId', authenticate, async (req, res, next) => {
  try {
    ...
    // 移除成员
    await execute(
      'UPDATE users SET family_id = NULL WHERE id = $1::text AND family_id = $1',
      [memberId, id]
    );
...
router.post('/:id/transfer-admin', authenticate, async (req, res, next) => {
  try {
    ...
    // 检查新管理员是否是家庭成员
    const members = await query(
      'SELECT id FROM users WHERE id = $1::text AND family_id = $1',
      [newAdminId, id]
    );
    ...
    // 更新管理员
    await execute('UPDATE families SET admin_id = $1::text WHERE id = $1::text', [newAdminId, id]);
    await execute('UPDATE users SET role = $1::text WHERE id = $1::text', ['admin', newAdminId]);
    await execute('UPDATE users SET role = $1::text WHERE id = $1::text', ['member', req.user.id]);
...
router.get('/qrcode', authenticate, async (req, res, next) => {
  try {
    ...
    const families = await query('SELECT invite_code, name FROM families WHERE id = $1::text', [req.user.familyId]);
