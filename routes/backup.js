...
router.get('/create', authenticate, async (req, res, next) => {
  try {
    ...
    const [family, members, bills, posts] = await Promise.all([
      ...
      query(
        'SELECT id, username, nickname, email, role, status, created_at FROM users WHERE family_id = ?',
        [familyId]
      ),
      query(
        'SELECT * FROM bills WHERE family_id = ? ORDER BY date DESC',
        [familyId]
      ),
      query(
    ...
    try {
      comments = await query(
        'SELECT * FROM post_comments WHERE post_id IN (SELECT id FROM posts WHERE family_id = ?)',
        [familyId]
      );
      likes = await query(
        'SELECT * FROM post_likes WHERE post_id IN (SELECT id FROM posts WHERE family_id = ?)',
        [familyId]
      );
    } catch (_e) {
      // 忽略帖子相关表不存在的错误
...
router.get('/preview', authenticate, async (req, res, next) => {
  try {
    ...
    const [billCount, memberCount] = await Promise.all([
      query('SELECT COUNT(*) as count FROM bills WHERE family_id = ?', [familyId]),
      query('SELECT COUNT(*) as count FROM users WHERE family_id = ?', [familyId])
    ]);

    let postCount = [{ count: 0 }];
    try {
      postCount = await query('SELECT COUNT(*) as count FROM posts WHERE family_id = ?', [familyId]);
    } catch (_e) { /* 忽略 */ }
...
router.post('/restore', authenticate, async (req, res, next) => {
  try {
    ...
    if (mode === 'overwrite') {
      // 清空现有账单
      await execute('DELETE FROM bills WHERE family_id = ?', [familyId]);
    }
    ...
    for (const bill of backupData.bills) {
      try {
        ...
        if (mode === 'merge') {
          // 检查是否已存在（按 id 去重）
          const existing = await query('SELECT id FROM bills WHERE id = ? AND family_id = ?', [bill.id, familyId]);
